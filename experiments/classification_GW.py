...
# from a5037_course_work.src.models.WSI_graph_walk import WSIGraphClassifier, GraphWalker
...
# def train_and_evaluate_kfold(data, task, encode_name, embedding_dim=2048,
...
#                         pooled, visit_count = walker.analyze(coords, embeddings, threshold=threshold, visual=False)
#                         pools.append(pooled)
...
#     data = []
#     for fname in tqdm(os.listdir(folder)[:20], desc=f"Loading data from {folder}."):
...
#     # python3 a5037_course_work/experiments/classification_GW.py --encode_name resnet --task site 
...
def train_and_evaluate_kfold(data, fnames, task, encode_name, embedding_dim,
                              batch_size=4, threshold=0.1, epochs=10, lr=1e-3,
                              log_path="experiment_log.txt", k_folds=5):
    assert task in ['site', 'subtype'], "Task must be either 'site' or 'subtype'."
    num_classes = 10 if task == 'site' else 29
    ...
    # 创建缓存目录
    cache_dir = f"a5037_course_work/data/cache_pooled_embeddings_{encode_name}"
    os.makedirs(cache_dir, exist_ok=True)

    walker = GraphWalker(distance=224)

    def get_pooled(fname, node_list):
        cache_path = os.path.join(cache_dir, fname.replace(".json", ".pt"))
        if os.path.exists(cache_path):
            pooled = torch.load(cache_path)
        else:
            coords = [(d['location']['x'], d['location']['y']) for d in node_list]
            embeddings = torch.stack([torch.tensor(d['embedding'], dtype=torch.float32) for d in node_list])
            pooled, _ = walker.analyze(coords, embeddings, threshold=threshold, visual=False)
    ...
    inputs = []
    targets = []
    for sample, fname in zip(data, fnames):
        feature = [{
            "location": d['location'],
            "embedding": d['embedding']
        } for d in sample]

        label = SITE2LABEL[sample[0][task]] if task == "site" else SUBTYPE2LABEL[sample[0][task]]
        inputs.append((feature, fname))
        targets.append(int(label))
    ...
    all_train_accs, all_val_accs, all_fold_losses = [], [], []
    task_val_accs_dict = {}
    all_val_class_correct = [0 for _ in range(num_classes)]
    all_val_class_total = [0 for _ in range(num_classes)]
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(inputs, targets)):
        ...
        fold_train_accs, fold_val_accs, fold_losses = [], [], []
        ...
        with tqdm(range(1, epochs + 1), desc=f"Fold {fold+1}/{k_folds}", ascii=True) as pbar:
            for epoch in pbar:
                ...
                for i in range(0, len(train_inputs), batch_size):
                    ...
                    pools = [get_pooled(fname, node_list) for node_list, fname in batch_data]
                    batch_pool = torch.stack(pools, dim=0).to(device)
                    labels_tensor = torch.tensor(batch_labels, dtype=torch.long).to(device)

                    logits_tensor = classifier(batch_pool)
                    loss = criterion(logits_tensor, labels_tensor)
        ...
        all_val_accs.append(fold_val_accs)
        all_fold_losses.append(fold_losses)
        task_val_accs_dict.setdefault(task, []).append(fold_val_accs)
...
def main(args):
    data_folder = f"a5037_course_work/data/embeddings/{args.encode_name}"
    data, fnames = load_json_data(data_folder)

    log_path = args.log_path or f"a5037_course_work/experiments/results/{args.encode_name}_GW_{args.task}_results.txt"

    embedding_dims = {"resnet": 2048, "vit": 768, "dino": 768, "uni": 1024}
    embedding_dim = embedding_dims[args.encode_name]
