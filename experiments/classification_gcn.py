...
def train_and_evaluate_kfold(data, model, task, encode_name, embedding_dim=2048,
        ...
        with tqdm(range(1, epochs + 1), desc=f"Fold {fold+1}/{k_folds}", ascii=True) as pbar:
            for epoch in pbar:
                ...
                for i in range(0, len(train_inputs), batch_size):
                    ...
                    all_logits = []
                    for node_list in batch_data:
                        coords = [(d['location']['x'], d['location']['y']) for d in node_list]
                        embeddings = torch.stack([
                            torch.tensor(d['embedding'], dtype=torch.float32) for d in node_list
                        ]).to(device)

                        adj, sim_dict = model._build_graph(coords, embeddings)
                        edge_index = model._build_edge_index(adj).to(device)
...
if __name__ == "__main__":
    ...
    # python3 a5037_course_work/experiments/classification_gcn.py --encode_name resnet --task site 